...
REQUEST_WAIT = 10
BREQ_DIR = "/home/sysop/breqfast2/breq"
SPOOL_DIR = "/home/sysop/breqfast2/spool"
...
FTP_DIR = "/srv/ftp/breqfast"
FTP_URL = "ftp://ftp.webdc.eu/breqfast"
ACC_DIR = "/home/sysop/breqfast2/lib"
LABEL = "breq_req"


VERSION = "0.6 (2010.256)"
	...
	def __parse_request(self, line):
		...
		if m:
			...
			for (network, station) in self.__expand_net_station(d["network"], d["station"], beg_time, end_time):
				...
				for cha in cha_list:
					cha = re.sub("[?]+","*",cha)
					self.reqlist.append((str(network),str(station),cha,loc,beg_time,end_time,{}))
...
_noktext = "Your breq_fast request to the WebDC has been processed. We are very sorry, but unfortunately none of the requested data windows have been found!"
...
def check_request(fname, basename, parser):
	...
	sys.stderr.write( "--> total (calclulated) size of request: %f MByte\n" % (requestSize / 1024.0**2))
	if requestSize > (1.0 * 1024.0**3):
		sys.stderr.write( "--> this request is too large!!\n")
		msg = _toolarge + "\n----> reason for refusal: request too large! (%.1f > 1.0 GByte)" % (requestSize / 1024.0**3)
...
def evaluate_status(req_sent, ok, failed):
	"""
	The given request status will be evaluated.

	@arguments: req_sent,   list of submitted requests
				ok,      a list filled with the successfully processed request lines
				failed,  a dictionary filled with the failed request lines associated to a status value
	@return: True, if there is data found to download; False otherwise
	"""
	retval = False
...
def submit_request(parser, req_name, breq_id):
	...
	try:
		...
		try:
			(files, inv, req_sent, req_noroute, req_nodata) = arcl.get_data(wf_req, True, True)
		...
		try:
			...
			evaluate_status(req_sent, ok_content, failed_content)

			if len(files) > 0:
				if not os.path.exists("%s/%s" % (FTP_DIR, req_name)):
					os.mkdir("%s/%s" % (FTP_DIR, req_name))
				dname = "%s/%s/%s_%s_archive.seed" % (FTP_DIR, req_name, label, breq_id)
				dname_extern = "%s/%s/%s_%s_archive.seed" % (FTP_URL, req_name, label, breq_id)
		
				try:
					if len(files) > 1:
						# Rebuild SEED volume

						seed_volume = SEEDVolume(inv, ORGANIZATION, SEED_LABEL)

						for fh in files:
							fh.seek(0)
							for rec in mseed.Input(fh):
								seed_volume.add_data(rec)
									
						fh_out = file(dname,"w")
						try:
							seed_volume.output(fh_out)
						...
						try:
							for fh in files:
								fh.seek(0)
						    	shutil.copyfileobj(fh, fh_out)

						finally:
							fh_out.close()
									
				except (IOError, ArclinkError, socket.error, OSError, DBError, SEEDError, mseed.MSeedError), e:
					logs.error("error creating SEED Volume: %s" % str(e))
					try:
						failed_content[STATUS_SEED] += ok_content
					except KeyError:
						failed_content[STATUS_SEED] = ok_content
	...
	rqlines = ""
	if len(ok_content) > 0:
		emailmsg = "%s\n\n%s\n\n%s\n\n%s\n" % (_oktext1,dname_extern,_oktext2,_twok)
	...
	if len(failed_content) > 0:
		...
		for keystr in failed_content:
			if keystr != STATUS_NODATA:
				emailmsg = "%s%s\n" % (emailmsg,_status_to_string(keystr))
