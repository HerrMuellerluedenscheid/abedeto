...
class _Request(object):
    ...
    def download_data(self, fd, block=False, purge=True):
        if self.id is None:
            raise ArclinkError, "request not submitted"
        ...
        while True:
            self.__arcl.open_connection(self.__host, self.__port,
                self.__user, self.__passwd, None, self.socket_timeout)

            try:
                try:
                    self.__arcl.download_data(fd, self.id, timeout=tmo,
    ...
    def download_xml(self, db, block=False, purge=True):
        ...
        while True:
            ...
            try:
                try:
                    self.__arcl.download_xml(db, self.id, timeout=tmo,
                        decomp=self.args.get("compression"))
...
class RequestThread(threading.Thread):
    def __init__(self, req, addr, user, passwd, user_ip, req_sent, req_retry, files):
    ...
    def run(self):
        try:
            ...
            try:
                ...
                logs.warning("%s: error: %s" % (self.req.address, str(e)))
...
class ArclinkManager(object):
    ...
    def __expand_request(self, req, inv):
        ...
        for i in req.content:
            for j in i[:4]:
                if j.find("*") >= 0 or j.find("?") >= 0 or j[0] in inv.stationGroup:
                    break
            else:
                continue
    ...
    def __get_data(self, db, request, req_sent, req_noroute, req_nodata, files):
        def _cmptime(t1, t2):
        ...
        for item in request.content:
            try:
                route = db.route[item[0]][item[1]][item[2]][item[3]]
            except KeyError:
                try:
                    route = db.route[item[0]][item[1]][item[2]][""]
                except KeyError:
                    try:
                        route = db.route[item[0]][item[1]][""][""]
                    except KeyError:
                        try:
                            route = db.route[item[0]][""][""][""]
        ...
        for (addr, req) in req_route.items():
            ...
            thr = RequestThread(req, addr, user, passwd, self.__user_ip, req_sent, req_retry, files)
            thr.start()
            req_thr.append(thr)
        ...
        if req_retry.content:
            return self.__get_data(db, req_retry, req_sent, req_noroute, req_nodata, files)
        
    def get_data(self, request, use_inventory = True, use_routing = True):
        ...
        if use_routing:
            ...
            self.__get_data(rtn, request, req_sent, req_noroute, req_nodata, files)
            ...
            if request.error is not None:
                raise ArclinkError, "error getting waveform data from %s: %s" % \
                    (request.address, request.error)
            
            fd = NamedTemporaryFile()
            files.append(fd)

            request.download_data(fd, True, False)
            request.status()
            request.purge()
        ...
        return (files, inv, req_sent, req_noroute, req_nodata)
    ...
    def __route_request(self, db, blacklist, req_ok, request):
        ...
        for item in request.content:
            ...
            # FIXME: effective start and end time not checked
            server_list = sum([i.values() for i in route.arclink.itervalues()], [])
            server_list.sort()
            arclink_addrs = []
            for server in server_list:
