...
def make_plot(
    ...
    max_depth,
    xlim,
    store_id_mapping=None,
    combine=True,
):
    """create pretty model plots"""
    if store_id_mapping and not len(store_id_mapping) == len(store_ids):
        raise Exception(
            "If id mapping is used, it needs to have the same length " "as store_ids"
        )
    ...
    fig, axs = plt.subplots(
        len(parameters), wantrows, figsize=(wantrows * 5, len(parameters) * 3)
    )
    if combine:
        axs = [axs]
    if len(parameters) == 1:
    ...
    for i_store, store_id in enumerate(store_ids):
        ...
        for i_param, param in enumerate(parameters):
            ...
            if store_id_mapping:
                ...
                label = store_id
            ax.plot(mod.profile(param) / 1000.0, mod.profile("z") / 1000.0, label=label)
            ax.set_ylim([0, max_depth / 1000.0])
            if xlim:
                ax.set_xlim(xlim)
