...
class JMap(automap.Map):
    ...
    def _setup_gmt(self):
        ...
        gmtconf.update((k.upper(), v) for (k, v) in self.gmt_config.iteritems())
...
def make_map(
    ...
    if stations:
        for i_station, s in enumerate(stations):
            ...
            _map.gmt.psxy(in_columns=(lons, lats), S="t20p", G=color, *_map.jxyr)
    ...
    if events:
        shifts = [(-0.6, -0.6), (-0.6, 0.0)]
        for i_e, e in enumerate(events):
            if e.moment_tensor:
                _map.gmt.psxy(
                    in_columns=([e.lon], [e.lat]), S="a25p", G="red", *_map.jxyr
                )

                break

                size_cm = (
                    math.sqrt(math.sqrt(mag2mom(e.moment_tensor.magnitude) / 10e7))
                    * size_factor
                )
                m = e.moment_tensor
                # mc = uniso(m)
