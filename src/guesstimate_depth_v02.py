...
class PlotSettings(Object):
    trace_filename = String.T(
        help="filename of beam or trace to use for " "plotting, incl. path.",
        optional=True,
    )
    ...
    store_id = String.T(
        help="Store ID to use for generating the synthetic " "traces.", optional=True
    )
    store_superdirs = List.T(String.T(), optional=True)
    depth = Float.T(help="Depth [km] where to put the trace.", default=10.0)
    depths = String.T(
        help="Synthetic source depths [km]. start:stop:delta. " "default: 0:15:1",
        optional=True,
        default="0:15:1",
    )
    filters = List.T(
        FrequencyResponse.T(help="List of filters used to filter " "the traces")
    )
    zoom = List.T(
        Float.T(),
        help="Window to visualize with reference to the P " "phase onset [s].",
        default=[-7, 15],
    )
    ...
    save_as = String.T(
        default="depth_%(array-id)s.png", help="filename of output figure"
    )
    force_nearest_neighbor = Bool.T(
        help="Handles OutOfBounds exceptions. " "applies only laterally!", default=False
    )
    auto_caption = Bool.T(
        help="Add a caption giving basic information to the figure.", default=False
    )
    title = String.T(default="%(array-id)s - %(event_name)s", help="Add default title.")
    quantity = String.T(
        default="velocity",
        help="velocity-> differentiate synthetic." "displacement-> integrate recorded",
    )
...
class GaussNotch(FrequencyResponse):
    ...
    def evaluate(self, freqs):
        denom = self.fwhm / (2.0 * math.sqrt(math.log(2.0)))
        return 1.0 - num.exp(1 - ((freqs - self.center) / denom) ** 2)
...
def notch_filter(tr, f_center, bandwidth):
    ...
    FFF = num.ones((fft_length), dtype=num.complex) * GaussNotch(
        f_center, bandwidth
    ).evaluate(freqs)
...
def plot(settings, show=False):
    ...
    station = list(
        filter(
            lambda s: match_nslc("%s.%s.%s.*" % s.nsl(), traces[0].nslc_id), stations
        )
    )
    assert len(station) == 1
    station = station[0]
    targets = [
        station_to_target(station, quantity=quantity, store_id=settings.store_id)
    ]
    try:
        ...
        if settings.force_nearest_neighbor:
            ...
            for t in targets:
                ...
                max_dist_delta = (
                    store.config.distance_max - farthest_source.distance_to(t)
                )
                if min_dist_delta < 0:
    ...
    for s, t, tr in request.iter_results():
        ...
        if False:
            ax.fill_between(
                xdata, y_pos, ydata, where=ydata < y_pos, color="black", alpha=0.5
            )
        ax.text(
            zoom_window[0] * 1.09,
            y_pos,
    ...
    if len(traces) > 1:
        ...
        tr = settings.do_filter(tr)
        onset = (
            engine.get_store(targets[0].store_id).t(
                "begin", (event.depth, s.distance_to(targets[0]))
            )
            + event.time
        )
        if settings.normalize:
        ...
        if ampl_scaler == "trace min/max":
            ...
            ampl_scale = 1.0
        ydata = (
            tr_ydata / ampl_scale * settings.gain * settings.gain_record
        ) * relative_scale + y_pos
    ...
    if fill:
        ax.fill_between(
            xdata, y_pos, ydata, where=ydata < y_pos, color=settings.color, alpha=0.5
        )
    if with_onset_line:
    ...
    def invert(self, args):
        ...
        for array_id in self.provider.use:
            ...
            test_depths = num.arange(
                float(zstart) * km, float(zstop) * km, float(inkr) * km
            )
            traces = io.load(settings.trace_filename)
            event = model.load_events(settings.event_filename)
            assert len(event) == 1
            ...
            targets = [
                station_to_target(
                    station, quantity=settings.quantity, store_id=settings.store_id
                )
            ]
            try:
                request = engine.process(targets=targets, sources=test_sources)
                ...
                if settings.force_nearest_neighbor:
                    ...
                    for t in targets:
                        closest_source = min(
                            test_sources, key=lambda s: s.distance_to(t)
                        )
                        farthest_source = max(
                            test_sources, key=lambda s: s.distance_to(t)
                        )
                        min_dist_delta = (
                            store.config.distance_min - closest_source.distance_to(t)
                        )
                        max_dist_delta = (
                            store.config.distance_max - farthest_source.distance_to(t)
                        )
            ...
            tstart = (
                self.provider.timings[array_id].timings[0].t(mod, (event.depth, dist))
                + event.time
            )
            tend = (
                self.provider.timings[array_id].timings[1].t(mod, (event.depth, dist))
                + event.time
            )
            ref = ref.chop(tstart, tend)
            misfits = []

            center_freqs = num.arange(1.0, 9.0, 4.0)
            ...
            for i_fc, fc in enumerate(center_freqs):
                ...
                fl_min = fc - fc * 2.0 / 5.0
                fr_max = fc + fc * 2.0 / 5.0
                ...
                for i_width, width in enumerate(widths):
                    ...
                    for source, candidate in candidates:
                        ...
                        tstart = (
                            self.provider.timings[array_id]
                            .timings[0]
                            .t(mod, (source.depth, dist))
                            + event.time
                        )
                        tend = (
                            self.provider.timings[array_id]
                        ...
                        filters = [
                            ButterworthResponse(
                                corner=float(fc + width * 0.5), order=4, type="low"
                            ),
                            ButterworthResponse(
                                corner=float(fc - width * 0.5), order=4, type="high"
                            ),
                        ]
                        ...
                        m, n, aproc, bproc = ref.misfit(
                            candidate=candidate, setup=settings.misfit_setup, debug=True
                        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--store", dest="store_id", help="name of store id", required=True
    )
    parser.add_argument(
        "--store-superdir",
        help="store superdir",
    ...
    parser.add_argument(
        "--pick", help="name of file containing p marker", required=False
    )
    parser.add_argument(
        "--depth", help="assumed source depth [km]", default=10.0, required=False
    )
    parser.add_argument(
        "--depths", help="testing depths in km. zstart:zstop:delta", required=True
    )
    parser.add_argument(
        "--quantity", help="velocity|displacement", default="velocity", required=False
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--save-as", dest="save_as", help="file to store image", required=False
    )
    parser.add_argument(
        "--print_parameters",
    ...
    parser.add_argument("--title", help="title of figure", required=False)
    parser.add_argument(
        "--no-y-axis",
    ...
    parser.add_argument(
        "--show", help="Show results right away", action="store_true", required=False
    )
    parser.add_argument(
        "--settings",
    ...
    parser.add_argument(
        "--auto-caption", help="caption basic info", action="store_true", required=False
    )
    ...
    parser.add_argument(
        "--zoom", help="Zoom window like t1:t2", nargs=2, type=float, required=False
    )
